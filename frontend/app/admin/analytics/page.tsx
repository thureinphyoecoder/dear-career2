      ...
      <Card className="rounded-2xl border-border/70 bg-white shadow-none">
        <CardContent className="grid gap-0 p-0">
          {summary.top_paths.length === 0 ? (
            <p className="px-6 py-6 text-[0.92rem] text-[#727975]">No visitor data yet.</p>
          ) : (
            summary.top_paths.map((item) => (
              ...
              >
                ...
                <span className="text-sm text-[#727975]">{item.visitors} visitors</span>
                <span className="text-sm text-[#727975]">{item.visits} visits</span>
                <span className="text-sm text-[#727975]">
                  {item.last_seen_at ? new Date(item.last_seen_at).toLocaleString() : "No recent visit"}
                </span>
              </div>
            ))
          )}
